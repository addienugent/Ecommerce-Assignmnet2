...
namespace MyApp // Note: actual namespace depends on the project name.
{
    internal class Program
    {
        ...
        private static void PrintInventory(ProductServices inventory)   // Prints Inventory 
        {
            ...
            /*foreach (var prod in newinventory)
            {
        ...
        // ----------------------------------------------------------------------------------------
        //               untouched working versio on print inventory below
        // ----------------------------------------------------------------------------------------
        ...
        /*
                private static void PrintInventory(ProductServices inventory)   // Prints Inventory 
                {
                    ...
                    IEnumerable<Product>? newinventory = null;
                    if (sortType == 1)   // Name Sort
                    {
                        newinventory = inventory.Products.OrderBy(p => p.Name);//.ToList();

                    }
                    else if (sortType == 2)  // Price Sort
                    {
                        newinventory = inventory.Products.OrderBy(q => q.priceinp);
                    }
                    else
                    {
                        newinventory = inventory.Products;//.Products.OrderBy(p => p);//.ToList();
        ...
        private static void PrintCart(CartService shoppingcart)     // Prints shopping cart
        {
            ...
            /*IEnumerable<Product>? newcart = null;
            ...
            {
                Console.Write("How would you like your Cart Sorted By:\n1) Name\n2) Total Price\nEnter choice: ");
                var sortType = int.Parse(Console.ReadLine() ?? "0");
                if (sortType == 1)   // Name Sort
                {
                    newcart = shoppingcart.Cart.OrderBy(p => p.Name);
                ...
                {
                    newcart = shoppingcart.Cart;    // No Sort - Invaild option picked prints as is
                    Console.WriteLine("Invalid Sort Type Printing as is. ");
                }
                ...
                Console.WriteLine($"---------------------------------------------------------------------------------");
                Console.WriteLine($"ID#:\tName:\t\tUnits:\t\tTotal Price:");
                Console.WriteLine($"---------------------------------------------------------------------------------");

                foreach (var prod in newcart)
                {
                    if (prod is ProductByQuantity)
                    {
                        var pt = prod as ProductByQuantity;
                        if (pt != null)
                            Console.WriteLine($"{pt.Id}\t{pt.Name}\t\t{pt.Quantity}\t\t$ {pt.getTotalPrice()}");
                    }
                    else if (prod is ProductByWeight)
                    {
                        var pt = prod as ProductByWeight;
                        if (pt != null)
                            Console.WriteLine($"{pt.Id}\t{pt.Name}\t\t{pt.Weight}\t\t$ {pt.getTotalPrice()}");
                    }
                }
            }*/
        }
